...
getAllBranches(){
    sudo owner=$1 siteRepo=$2 dataRepo=$3 githubToken=$4 docker-compose up outdatedbranches
}
...
buildSiteBranches(){
    ...
    do
        ...
        then
            ...
            sudo branch=$branchName docker-compose up sitebuild > ~/.featbranch/server/logs/$1_$branchName.txt
            ...
            then
                ...
                sudo branch=$branchName docker-compose up a11yreport
                ...
                sudo message="$slackMessage" webHook="$slackWebhook" docker-compose up slack
            fi
        fi
    done
}
...
buildDataBranches(){
    ...
    do
        ...
        then
            ...
            sudo branch=$branchName docker-compose up databuild > ~/.featbranch/server/logs/$1_$branchName.txt
            curl -X POST -d "repository=$1&branch=$branchName&latestCommit=$branchLatestCommit" http://localhost:5001/add-entry

            if [ $(<~/.featbranch/output/databuildSuccess.out) = "1" ];
            then
                sudo dataBranch=$branchName serverUrl=$serverUrl docker-compose up datapreviewbuild > ~/.featbranch/server/logs/datapreview_$branchName.txt
